...
`comparator_scope=primary` restricts legacy comparators to configured primary comparisons and evidence tiers. Scope-matched method comparisons should use this setting. `comparator_scope=all_eligible` is a diagnostic option that exposes otherwise eligible strata to legacy comparators; it must not be compared causally with a method restricted to primary scope.
